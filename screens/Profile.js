...
export default function Profile({ loggedUser, goals=[], journalEntries=[] }) {
  const user = loggedUser || {};
  const preferences = user.preferences || { notifications: false, theme: "Light" };
  ...
  return (
    <ScrollView   style={{ flex: 1, backgroundColor: appStyles.screen.backgroundColor }}
     ...
     padding: appStyles.screen.padding, }}>
      <View
        style={[appStyles.avatar, { backgroundColor: avatarColor }]}>
      ...
      <View style={appStyles.preferences}>
